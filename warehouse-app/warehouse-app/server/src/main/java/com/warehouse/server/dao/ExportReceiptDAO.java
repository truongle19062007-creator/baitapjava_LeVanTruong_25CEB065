...
public class ExportReceiptDAO {
    ...
    private static final String BASE_SELECT =
            ...
            "FROM export_receipts r JOIN users u ON r.created_by = u.id ";

    public List<ExportReceipt> findAll() throws SQLException {
    ...
    public Optional<ExportReceipt> findById(Long id) throws SQLException {
        ...
        try (Connection conn = DatabaseManager.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                ExportReceipt receipt = mapRow(rs);
    ...
    public List<ReceiptItem> findItems(Connection conn, Long receiptId) throws SQLException {
        String sql = "SELECT ri.*, p.code AS product_code, p.name AS product_name " +
                "FROM export_receipt_items ri JOIN products p ON ri.product_id = p.id " +
                "WHERE ri.export_receipt_id = ?";
    ...
    public ExportReceipt insertHeader(Connection conn, ExportReceipt receipt) throws SQLException {
        String sql = "INSERT INTO export_receipts (code, customer_name, created_by, note, total_amount) " +
                "VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ps.setString(1, receipt.getCode());
            ps.setString(2, receipt.getCustomerName());
    ...
    public void insertItem(Connection conn, Long receiptId, ReceiptItem item) throws SQLException {
        String sql = "INSERT INTO export_receipt_items (export_receipt_id, product_id, quantity, price) " +
                "VALUES (?, ?, ?, ?)";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setLong(1, receiptId);
    ...
    public void delete(Long id) throws SQLException {
        String sql = "DELETE FROM export_receipts WHERE id = ?";
        try (Connection conn = DatabaseManager.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setLong(1, id);
            ps.executeUpdate();
    ...
    public String generateNextCode() throws SQLException {
        String sql = "SELECT COUNT(*) AS cnt FROM export_receipts";
        try (Connection conn = DatabaseManager.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            int count = rs.next() ? rs.getInt("cnt") : 0;
            return String.format("PX%05d", count + 1);
        }
    }
